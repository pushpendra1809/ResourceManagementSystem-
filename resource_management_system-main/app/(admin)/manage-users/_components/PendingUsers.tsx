...
import { approveUser, deleteUser } from "@/server-action/user";
...
const PendingUsers = ({
  ...
  nonVerifiedUser: UserModelType[];
  setNonVerifiedUser: Dispatch<SetStateAction<UserModelType[]>>;
}) => {
  const deleteHandler = async (id: ObjectId) => {
    const res = await deleteUser(id);
    if (res.success) {
      setNonVerifiedUser(nonVerifiedUser.filter((user) => user._id !== id));
      toast.success(res.message);
    } else toast.error(res.message);
  };
  const approveHandler = async (id: ObjectId) => {
  ...
  return (
    <div className="w-full flex flex-col gap-5 h-[600px] overflow-auto pr-5 py-3">
      {nonVerifiedUser.map((user, index) => (
        <Card key={index} className="w-full px-5 py-2 grid grid-cols-12">
          ...
          <div className="flex flex-col items-end col-span-1">
            <div className="">
              ...
              >
                <FaCheck className="w-5 h-5 cursor-pointer text-muted-foreground group-hover:text-primary transition-all duration-300 ease-in-out" />
              </Button>
            </div>
            <Dialog>
              <DialogTrigger>
